...
# Custom tab name
st.set_page_config(
    page_title="Digital Infrastructure Gap Portal",  
    page_icon="🌐", 
    layout="wide"  
)
...
# **Chatbot**
# Loading a text generation model for chatbot via huggingface api
@st.cache_resource
def load_mistral_api():
    """ Load and return a Mistral-7B model client via huggingface api """
    load_dotenv()
    api_token = os.getenv("HUGGINGFACE_API_KEY")
    
    if not api_token:
        st.error("Hugging Face API token is missing. Please set it correctly.")
        return None
...
context_data = """
The Digital Infrastructure Gap Portal empowers citizens by providing a centralized platform to enhance transparency in urban development.

Available features:
1. Voting System: 
- Citizens can vote on proposed projects through the "Your City, Your Choice" initiative. 
- Voting is accessible from the 'Home' page during an ongoing voting session if the citizen hasn't voted yet. 
- Users can select a project from a list of voting-open or proposed projects and submit their vote.
- After submitting the vote, a confirmation message stating "Vote recorded successfully" is displayed.
- Only one vote is permitted per user per project and one vote per voting session.

2. Facility Request System:
- Citizens can raise facility requests from the 'My Facility Requests' page by accessing the 'Raise a New Facility Request' form.
- They need to select a category (e.g., Roads, Water Supply, Electricity, Parks, Cleaning), provide a description, and optionally upload an image for context.
- After submitting the request, they receive a confirmation message stating, "Request submitted successfully!"
- Status updates on requests include: Pending, Approved, Completed, and Rejected.

3. Feedback Mechanism:
- Citizens can submit feedback on active project from the 'Home' page and on completed facility requests from the 'My Facility Requests' page.
- Feedback can be submitted for either a project or a facility request at a time.

4. Viewing and Tracking Requests:
- Citizens can track the status of their facility requests in the 'My Facility Requests' section.
- Status updates (Pending, Approved, Completed, or Rejected) are sent as notifications to keep users informed about any changes.

The portal promotes active citizen engagement by encouraging participation in decision-making and ensuring accountability in service delivery.

"""


def get_answer(question, context):
    """ Return answer/response from Mistral-7B API """
    mistral_api = load_mistral_api()

    if not mistral_api:
        st.error("Mistral API could not be loaded.")
    
    if "status" in question.lower():
        status_data = fetch_request_data_for_status()
        updated_context = context_data + "\n\nFacility Request Information:\n" + status_data
        
        status_response = extract_request_status_info(question, updated_context)
    ...
    try:
        response = mistral_api.text_generation(
            prompt,
            max_new_tokens=500,
            temperature=0.3,
            do_sample=False
        ...
        return answer if answer else "I'm sorry, I couldn't find an answer."
    
    except Exception as e:
        return f"Error: {str(e)}"
      
    
def fetch_request_data_for_status():
    """ fetch facility request data from API for the status enquiry through chatbot """
    request_data = get_facility_requests()
    
    if request_data:
        formatted_requests = []
        for req in request_data:
            formatted_requests.append(f"Request ID: {req['id']} - {req['description']} - Status: {req['status']}")
        return "\n".join(formatted_requests)
    
    return "No facility request data available."


def extract_request_status_info(question, context):
    """ Extracts request status info """
    request_id = None
    ...
    if request_id:
        for line in context.split("\n"):
            if f"Request ID: {request_id}" in line:
                return line
    
    return "I couldn't find any status information for that request. Please check the request ID."
        
        
def show_chatbot():
    """ Displays the chatbot """
    st.subheader("🤖 AI ChatBot") 
    
    # Initializing chat_history if not already in session state
    ...
    if user_query := st.chat_input("Ask me anything about the portal..."):
        # Append user message to the chat history
        st.session_state.chat_history.append({"role": "user", "content": user_query})
        
        # Get bot response
        with st.spinner("Working on it..."):
            response = get_answer(question=user_query, context=context_data)
            
        # Append bot reponse to the chat history
        st.session_state.chat_history.append({"role": "assistant", "content": response})
        st.rerun() # Reruns the script to re render all the messages, ensuring input field stays at the bottom
    ...
    if active_projects:
        ...
        for project in active_projects:
            with st.expander(f"{project['name']} - {project['status']}"):
                ...
                st.write(f"**Created At:** {format_datetime(project['created_at'])}")
    ...
    if rejected_requests:   
        ...
        for req in rejected_requests:
            with st.expander(f"Request ID: {req['id']} - {req['category']} (Submitted by {req['username']})"):
                st.write(f"**Description:** {req['description']}")
                st.write(f"**Status:** {req['status']}")
                st.write(f"**Created At:** {format(req['created_at'])}")
...
def user_feedback_analytics(feedback_df):   
    ...
    with st.expander("View Detailed Feedbacks"):
...
def show_admin_dashboard():
    ...
    tab1, tab2, tab3 = st.tabs(["Voting Analytics", "Request Analytics", "User Feedback Analytics"])
...
def show_user_dashboard():
    ...
    if voting_projects:
        if not user_already_voted():
            st.subheader("Vote for a Project | Your City, Your Choice !")
        ...
        st.info("No proposed projects to vote")
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        if st.session_state.username != "admin":    
            ...
            # Opens chatbot in a popover 
            with st.popover("💬 Ask AI"):
                show_chatbot()  # Display the chatbot UI within the popover
